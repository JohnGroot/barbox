...
public class MiningEventService : GameEventServiceBase
{
	...
	public MiningEventService(SessionEventService eventService = null)
		: base(eventService)
	{
	}
	...
	private Result<MiningMetadataData> ParseMetadataData(Godot.Collections.Dictionary jsonDict)
	{
		...
		{
			return Result.Failure<MiningMetadataData>(playerIdError.Message);
		}

		if (!playerIdResult.IsSuccess(out var playerId))
		{
			return Result.Failure<MiningMetadataData>("Failed to extract player ID");
		}

		if (locationIdResult.IsFailure(out var locationIdError))
		{
			return Result.Failure<MiningMetadataData>(locationIdError.Message);
		}

		if (!locationIdResult.IsSuccess(out var locationId))
		{
			return Result.Failure<MiningMetadataData>("Failed to extract location ID");
		}

		if (firstEventTimeResult.IsFailure(out var firstEventTimeError))
		{
			return Result.Failure<MiningMetadataData>(firstEventTimeError.Message);
		}

		if (!firstEventTimeResult.IsSuccess(out var firstEventTime))
		{
			return Result.Failure<MiningMetadataData>("Failed to extract first event time");
		}

		if (lastEventTimeResult.IsFailure(out var lastEventTimeError))
		{
			return Result.Failure<MiningMetadataData>(lastEventTimeError.Message);
		}

		if (!lastEventTimeResult.IsSuccess(out var lastEventTime))
		{
			return Result.Failure<MiningMetadataData>("Failed to extract last event time");
		}
	...
	private Result<MiningStateData> ParseStateData(Godot.Collections.Dictionary jsonDict)
	{
		...
		if (playerIdResult.IsFailure(out var playerIdError))
		{
			return Result.Failure<MiningStateData>(playerIdError.Message);
		}

		if (!playerIdResult.IsSuccess(out var playerId))
		{
			return Result.Failure<MiningStateData>("Failed to extract player ID");
		}

		if (locationIdResult.IsFailure(out var locationIdError))
		{
			return Result.Failure<MiningStateData>(locationIdError.Message);
		}

		if (!locationIdResult.IsSuccess(out var locationId))
		{
			return Result.Failure<MiningStateData>("Failed to extract location ID");
		}

		if (inventoryResult.IsFailure(out var inventoryError))
		{
			return Result.Failure<MiningStateData>(inventoryError.Message);
		}

		if (!inventoryResult.IsSuccess(out var inventory))
		{
			return Result.Failure<MiningStateData>("Failed to extract inventory");
		}

		if (upgradesResult.IsFailure(out var upgradesError))
		{
			return Result.Failure<MiningStateData>(upgradesError.Message);
		}

		if (!upgradesResult.IsSuccess(out var upgrades))
		{
			return Result.Failure<MiningStateData>("Failed to extract upgrades");
		}

		if (lastExtractionTimeResult.IsFailure(out var timestampError))
		{
			return Result.Failure<MiningStateData>(timestampError.Message);
		}

		if (!lastExtractionTimeResult.IsSuccess(out var lastExtractionTime))
		{
			return Result.Failure<MiningStateData>("Failed to extract timestamp");
		}
